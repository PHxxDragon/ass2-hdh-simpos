	...
	while (index != 0) // node at root -> stop
	  {
		...
		if (q->proc[index]->prioprity <= q->proc[(index - 1) >> 1]->prioprity)
			break;
...
/*
	...
	TODO: return a pcb whose prioprity is the highestin the queue [q] and remember to remove it from q
*/
	...
	while ((index << 1) + 1 < q->size) // node is leaf -> stop
	  {
		...
		// if right child exists AND has higher prioprity than left child,
		// then [higherChild] is right child's index
		if (higherChild + 1 < q->size
			&& q->proc[higherChild]->prioprity < q->proc[higherChild + 1]->prioprity)
			++higherChild; // right child's index
			
		// if node [higherChild] has lower prioprity,
		// then the only maybe wrongly-placed node is placed right -> stop
		if (q->proc[higherChild]->prioprity < q->proc[index]->prioprity)
			break;
